...
function NavArrow({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/[0.08] bg-white/[0.04] text-white/40 hover:text-white hover:border-white/20 hover:bg-white/[0.07] transition-all"
    >
      {children}
    </button>
  )
    ...
    >
      ...
      <AnimatePresence mode="wait" custom={direction}>
        <motion.div
          ...
          className="relative z-10 grid grid-cols-1 lg:grid-cols-[1fr_260px]"
        >
          {/* ── Left column ── */}
          <div className="p-6 sm:p-8 md:p-10 flex flex-col gap-6">
            {/* Counter + tag */}
            <div className="flex items-center gap-3">
              <span className="hidden sm:block text-xs font-mono text-white/20 tracking-widest">
          ...
          {/* ── Right column ── */}
          <div
            className="flex flex-col gap-6 p-6 sm:p-8 md:p-10 border-t border-white/[0.06] lg:border-t-0 lg:border-l"
            style={{ borderColor: style.accentBorder }}
          >
            {/* Icon — desktop only */}
            <div
              className="hidden lg:flex h-20 w-20 items-center justify-center rounded-2xl self-end"
              style={{ background: style.accentDim, border: `1px solid ${style.accentBorder}` }}
            >
            ...
            <div className="flex flex-row lg:flex-col gap-6 flex-wrap">
              {c.stats.map((s) => (
                <div key={s.label}>
                  <p
                    className="text-3xl lg:text-4xl font-black text-white leading-none mb-1"
            ...
            <Link
              href={`/${locale}/agendar`}
              className="inline-flex items-center gap-2 self-start rounded-full px-5 py-2.5 text-xs font-semibold uppercase tracking-wide text-black bg-[#ffa845] hover:bg-[#f59e0b] transition-colors"
            >
              {t('bookCall')}
              <ArrowRight size={12} />
            </Link>
  ...
  return (
    <section id="casos-reales" className="relative py-28">
      <div className="relative z-10 mx-auto max-w-7xl px-6">
        ...
        {/* Desktop carousel — arrows flank the card */}
        <div className="hidden sm:flex items-center gap-4">
          <NavArrow onClick={prev}><ChevronLeft size={18} /></NavArrow>
          {card}
          <NavArrow onClick={next}><ChevronRight size={18} /></NavArrow>
        </div>

        {/* Mobile carousel — card full width, arrows + counter below */}
        ...
        {/* Tab navigation */}
        <div className="mt-6 flex items-center justify-center gap-1 flex-wrap sm:px-14">
          {cases.map((item, i) => {
