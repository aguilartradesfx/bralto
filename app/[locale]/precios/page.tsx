...
const SERVICE_HREFS: Record<string, (locale: string) => string> = {
  sitiosWeb:           (l) => `/${l}/servicios/sitios-web`,
  produccionContenido: (l) => `/${l}/servicios/produccion-contenido`,
  campanas:            (l) => `/${l}/servicios/campanas`,
  automatizacion:      (l) => `/${l}/servicios/automatizacion`,
  sistemasInternos:    (l) => `/${l}/servicios/sistemas-internos`,
  asesoria:            (l) => `/${l}/servicios/asesoria`,
  funnelLab:           ()  => 'https://funnellabs.bralto.io',
}
...
export default function PreciosPage() {
  ...
  const services = SERVICE_SLUGS.map((slug, i) => ({
    ...
    name:   t(`services.${slug}.name`),
    from:   t(`services.${slug}.from`),
    period: t(`services.${slug}.period`),
    desc:   t(`services.${slug}.desc`),
    href:   SERVICE_HREFS[slug](locale),
  }))
    ...
    <div className="min-h-screen bg-[#060607] text-white antialiased">
      ...
      <section className="grid-bg relative pt-40 pb-28 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12">
          ...
          >
            {t('headline')}
            <br />
            <em style={{ fontFamily: 'var(--font-instrument-serif)', fontStyle: 'italic', fontWeight: 400, color: 'rgba(255,255,255,0.25)' }}>
              {t('headlineItalic')}
            </em>
          </h1>
      ...
      <section className="relative py-40 overflow-hidden" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6 md:px-12 text-center">
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#5bb6ff] mb-6">{t('ctaLabel')}</p>
          <h2
            className="font-bold tracking-tight leading-[0.92] text-white mb-8"
            style={{ fontSize: 'clamp(2rem, 4vw, 3.5rem)' }}
          >
            {t('ctaHeadline')}<br />
            <span style={{ color: 'rgba(255,255,255,0.2)' }}>{t('ctaHeadlineItalic')}</span>
          </h2>
